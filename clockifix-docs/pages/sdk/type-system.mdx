...
<Callout type="info" title="Naming Rule">
  When a schema name is **unique** after stripping `Dto`, `DtoV1`, `DtoImplV1`, `V1` suffixes, we use the clean name (e.g., `WorkspaceDtoV1` becomes `Workspace`). When there's a **collision**, we preserve the raw OpenAPI name to avoid ambiguity.
</Callout>

## Using Types

Every type is exported from `clockifixed`:
