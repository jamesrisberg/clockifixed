...
Expense create and update require `multipart/form-data` content type, not `application/json`:

| Endpoint | Issue |
|---|---|
| `POST /workspaces/{id}/expenses` | Returns `415 Unsupported Media Type` with JSON |
| `PUT /workspaces/{id}/expenses/{id}` | Same — requires multipart form data |

The OpenAPI spec doesn't indicate the content type requirement.
