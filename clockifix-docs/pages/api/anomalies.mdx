...
| Nullable fields declared as required | 38+ unique fields | High — causes runtime validation failures |
...
|---|---|---|
| Endpoints checked | 45 (40 active) | 114 tests |
| Spec divergences (unique fields) | 30+ | 30+ |
| Undocumented response fields | 3 | 3 |
| Reality schema checks | 16/16 passing | 25/25 passing |
...
| `budgetEstimate` | `number` (required) | `null` |
| `costRate` | `object` (required) | `null` |
| `duration` | `string` (required) | `null` |
| `estimate` | `string` (required) | `null` |
| `hourlyRate` | `object` (required) | `null` |
...
| `costRate` | `object` (required) | `null` on read responses (WithRates) |
| `hourlyRate` | `object` (required) | `null` on read responses (WithRates) |
...
| Field | Spec says | Actually returns |
|---|---|---|
| `approve` | `object` (required) | `null` |
| `automaticAccrual` | `object` (required) | `null` |
...
| `userGroupIds` | `string[]` (required) | `null` |
| `userIds` | `string[]` (required) | `null` |
...
All 41 reality schema checks pass (16 read + 25 write), meaning Clockifixed's override types accurately model the real API behavior even where the spec is wrong.
